...
- Docker Hub: `nicholas/reactive-resume:latest`
- GitHub Container Registry: `ghcr.io/nicholas/reactive-resume:latest`
...
services:
  ...
  reactive-resume:
    image: nicholas/reactive-resume:latest
    # image: ghcr.io/nicholas/reactive-resume:latest
    restart: unless-stopped
    ports:
      - "3000:3000"
    env_file:
...
```
    ...
    <Tip>
      Prefer pulling from Docker Hub? Keep <code>nicholas/reactive-resume:latest</code>. Prefer GHCR? Swap it to <code>ghcr.io/nicholas/reactive-resume:latest</code>.
    </Tip>
  </Step>
...
```
  ...
  <Accordion title="Email (SMTP, optional)">
    ...
    - **`SMTP_FROM`**: Default from address (for example, `Reactive Resume <noreply@localhost>`).
