...
description: "A step-by-step guide to migrate your Ascend instance from v4 to v5, including manual and automated migration options."
---

## Overview

This guide walks you through migrating your Ascend installation from **v4 to v5**. The migration process involves setting up a new v5 instance alongside your existing v4 instance, then transferring your users and resumes to the new system.
...
<CardGroup cols={2}>
  <Card title="Running v4 Instance">
    Your existing Ascend v4 instance should be running and accessible.
  </Card>
  <Card title="New v5 Instance">
    A fresh Ascend v5 instance set up and running. Follow the [Self-Hosting with Docker](/self-hosting/docker) guide if you haven't done this yet.
  </Card>
...
<CardGroup cols={1}>
  ...
  <Card title="Ascend Source Code">
    Clone the Ascend repository to access the migration scripts:
    ```bash
    git clone #.git
    cd ascend
    pnpm install
...
DATABASE_URL="postgresql://user:password@localhost:5432/ascend_v5"

# Connection string to your OLD v4 PostgreSQL database (source)
PRODUCTION_DATABASE_URL="postgresql://user:password@localhost:5432/ascend_v4"
```
