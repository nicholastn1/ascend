...
Ascend supports custom OAuth providers, allowing you to integrate with enterprise identity providers and self-hosted authentication solutions. This is particularly useful for organizations that want to:
...
<Info>
  Custom OAuth is designed for **self-hosted instances**. If you're using the hosted version at [localhost](/), you can use the built-in Google and GitHub sign-in options.
</Info>
...
<Tabs>
  <Tab title="Option A: OIDC Discovery (Recommended)">
    ...
    **Examples:**
    - Authentik: `https://auth.example.com/application/o/ascend/.well-known/openid-configuration`
...
Ascend automatically maps user profile data from the OAuth provider. The following fields are used:

| Ascend Field | OAuth Profile Fields (in order of preference) |
|-----------------------|-----------------------------------------------|
...
OAUTH_DISCOVERY_URL="https://auth.example.com/application/o/ascend/.well-known/openid-configuration"
```
  </Step>
</Steps>
...
OAUTH_CLIENT_ID="ascend"
OAUTH_CLIENT_SECRET="your-plain-secret"
OAUTH_DISCOVERY_URL="https://auth.example.com/.well-known/openid-configuration"
```

    <Warning>
      Use the **plain text** secret in Ascend's environment, not the hashed version used in Authelia's configuration.
    </Warning>
  </Step>
</Steps>
...
OAUTH_CLIENT_ID="ascend-client-id"
OAUTH_CLIENT_SECRET="ascend-client-secret"
OAUTH_DISCOVERY_URL="https://auth.company.com/application/o/ascend/.well-known/openid-configuration"
...
<CardGroup cols={2}>
  <Card title="Use HTTPS" icon="lock">
    Always use HTTPS for both your Ascend instance and OAuth provider in production. OAuth tokens should never be transmitted over unencrypted connections.
  </Card>
  ...
  <Card title="Review scopes" icon="list-check">
    Only request the scopes you need. The default (`openid profile email`) is sufficient for Ascend.
  </Card>
</CardGroup>
