...
This example uses [Traefik](https://traefik.io/) as a reverse proxy with automatic SSL certificate management via Let's Encrypt. Only the Ascend app is exposed through Traefik—Postgres and the printer remain on an internal network.
...
services:
  ...
  ascend:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.ascend.rule=Host(`resume.${DOMAIN}`)"
      - "traefik.http.routers.ascend.entrypoints=websecure"
      - "traefik.http.routers.ascend.tls.certresolver=letsencrypt"
      - "traefik.http.services.ascend.loadbalancer.server.port=3000"
    healthcheck:
...
services:
  postgres:
    ...
    networks:
      - ascend_network
    volumes:
      - ascend_postgres_data:/var/lib/postgresql
    environment:
...
configs:
  ascend_postgres_config:
    name: ascend_postgres_config
    external: true
...
volumes:
  ascend_postgres_data:
    name: ascend_postgres_data
  ascend_seaweedfs_data:
    name: ascend_seaweedfs_data
  ascend_data:
    name: ascend_data
```
...
```bash
docker stack deploy -c compose-swarm.yml ascend
```
...
docker stack services ascend

# View logs for the app
docker service logs -f ascend_app

# Scale the app
docker service scale ascend_app=3

# Remove the stack
docker stack rm ascend
```
