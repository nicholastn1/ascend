...
<Info>
  If you're self-hosting, replace `/` with your instance URL. The API is served under `/api/openapi`.
</Info>
...
```bash
curl -X PATCH "//api/openapi/resume/YOUR_RESUME_ID" \
...
curl -X PATCH "//api/openapi/resume/YOUR_RESUME_ID" \
  ...
  -d '{
    "operations": [
      { "op": "replace", "path": "/metadata/template", "value": "mdi" },
